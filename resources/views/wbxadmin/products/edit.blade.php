                ...
                <div class="card-body p-5">
                  ...
                  <div class="form-colum">
                    <div class="mb-4">
                        ...
                        <input type="text" placeholder="News Title" class="form-control" id="titleInput" name="name" value="{{ $product['name'] }}" />
                    </div>
                    <div class="row" id="pagefields" style="display:block;">
                      <div class="mb-4">
                        ...
                        <div class="input-group input-group--sa-slug">
                          <span class="input-group-text" id="form-product/slug-addon" >{{ $settings->values }}/product/</span >
                          <input type="text" class="form-control" id="slugOutput" name="link" aria-describedby="form-product/slug-addon form-product/slug-help" value="{{ $product['link'] }}"/>
                        </div>
                      ...
                      <div class="mb-4">
                          <label for="meta-tittle" class="form-label">Meta Tittle @error('meta_title') <span class="text-danger"> * {{ $message }}</span> @enderror</label>
                          <input type="text" placeholder="Meta Title" class="form-control" id="meta_title" name="meta_title" value="{{ $product['meta_title'] }}" />
                      </div>
                      <div class="mb-4">
                          <label for="meta-keywords" class="form-label ">Meta Keywords @error('meta_keyword') <span class="text-danger"> * {{ $message }}</span> @enderror</label>
                              <textarea placeholder="Text Area" class="form-control" rows="3" id="meta_keyword" name="meta_keyword">{{ $product['meta_keyword'] }}</textarea> 
                      </div>
                      <div class="mb-3">
                          ...
                          <textarea placeholder="Text Area" class="form-control" rows="3" id="meta_description" name="meta_description">{{ $product['meta_description'] }}</textarea> 
                      </div>   
                      <div class="mb-4">
                          ...
                          <textarea class="form-control" name="short_description" id="editor1" cols="43" rows="5" class="textarea" tabindex="5" onkeydown="limitText(this.form.short_desc,this.form.countdown1,5000);"
                            onkeyup="limitText(this.form.product_desc,this.form.countdown1,5000);">{{ $product['short_description'] }}</textarea>
                      </div>
                      <div class="mb-4">
                          ...
                          <textarea class="form-control" name="long_description" id="editor2" cols="43" rows="5" class="textarea" tabindex="5" onkeydown="limitText(this.form.short_desc,this.form.countdown1,5000);"
                            onkeyup="limitText(this.form.product_desc,this.form.countdown1,5000);">{{ $product['long_description'] }}</textarea>
                      </div>
                    </div>
                    
                    <!-- <div class="row">
                      <div class="mb-3">
                          <label for="meta-tittle" class="form-label">Images @error('name') <span class="text-danger"> * {{ $message }}</span> @enderror</label>
                      </div>
                    </div> -->
                  </div>
                </div>
              ...
              <div class="card w-100">
                <div class="card-body p-5">
                  ...
                  <div class="mb-4">
                    ...
                    <input type="disabled" class="form-control" data-auto-close="true" data-language="en" value="{{ date('d-M-Y', strtotime($product['created_at'])) }}" />
                  </div>
                  <div class="mb-4">
                    <label for="form-product/seo-title" class="form-label" >Updated date</label > 
                    <input type="disabled" class="form-control" data-auto-close="true" data-language="en" value="{{ date('d-M-Y', strtotime($product['updated_at'])) }}" /><label class="form-check mb-0" >
                  </div>
                  <div class="mb-5">
                    <h2 class="mb-0 fs-exact-18">Product Categories</h2>
                  </div>
                  <div class="mb-4">
                      <label for="first-name" class="form-label">Select Category @error('name') <span class="text-danger"> * {{ $message }}</span> @enderror</label>
                ...
                <div class="card-body p-5">
                  ...
                  <div class="table-responsive">
                    <table class="sa-table" id="myTables">
                        ...
                        <a data-bs-toggle="offcanvas" href="#fileManagerDetails{{ $image['id']}}" ><img <?php if($image['default_status'] == 'Y'){ ?> style="border:5px solid #ffd333; width:100%; height:auto;" <?php } ?> src="{{ url('img/articles/products/'.$image['image']) }}" alt=""></a>
                          <tr>
                              <td>
...
@foreach($images as $image)
<form action="{{ route('product.imageupdate', $image['id'] ) }}" method="post" enctype="multipart/form-data" >
  @csrf
  @method('PUT')
<div class="offcanvas offcanvas-end offcanvas-sa-card" tabindex="-1" id="fileManagerDetails{{ $image['id']}}" aria-labelledby="fileManagerDetailsLabel">
  ...
  <div class="offcanvas-body" data-simplebar="">
      <div class="border p-4 d-flex justify-content-center mb-4">
        <div class="max-w-20x"><img src="{{ url('img/articles/products/'.$image['image']) }}" class="w-100 h-auto" width="320" height="320" alt=""/></div>
      </div>
      <div class="mb-4">
      ...
      <div class="hstack gap-3"><button type="submit" class="btn btn-primary flex-grow-1">Update</button><?php if($image['default_status'] == 'N'){ ?><a href="{{ route('product.imgdel', $image['id'] ) }}" class="btn btn-secondary flex-grow-1">Delete</a><?php } ?></div>
  </div>
</div>
</form>
@endforeach
