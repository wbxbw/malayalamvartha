...
<div id="top" class="sa-app__body">
  <div class="mx-sm-2 px-2 px-sm-3 px-xxl-4 pb-6">
    <div class="container">
      ...
      <form action="{{ route('blogs.update', $content['id']) }}" method="post" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-lg-12 col-md-12">
                            <div class="mb-4">
                                ...
                                <input type="text" placeholder="Blog Name" class="form-control" id="titleInput" name="name" value="{{ $content['name'] }}" />
                              </div>
                        </div>
                        <div class="col-lg-12 col-md-12">
                            <div class="mb-4">
                              ...
                              <div class="input-group input-group--sa-slug">
                                <span class="input-group-text" id="form-product/slug-addon" >{{ $settings->values }}/blogs/</span >
                                <input type="text" class="form-control" id="slugOutput" name="link" aria-describedby="form-product/slug-addon form-product/slug-help" value="{{ $content['link'] }}"/>
                              </div>
                        ...
                        <div class="col-lg-12 col-md-12">
                            <div class="mb-4">
                                ...
                                <input type="text" placeholder="Meta Title" class="form-control" id="meta_title" name="meta_title" value="{{ $content['meta_title'] }}" />
                              </div>
                        </div>
                    </div>
                    <div class = "mb-4">
                    <label for="meta-keywords" class="form-label ">Meta Keywords</label>
                        <textarea placeholder="Text Area" class="form-control" rows="3" id="meta_keyword" name="meta_keyword">{{ $content['meta_keyword'] }}</textarea> 
                    </div>
                    <div class = "mb-3">
                    <label for="meta-keywords" class="form-label">Meta Description</label>
                    <textarea placeholder="Text Area" class="form-control" rows="3" id="meta_description" name="meta_description">{{ $content['meta_description'] }}</textarea> 
                    </div>
                    <div class="mb-4">
                        ...
                        <textarea class="form-control" name="short_description" id="summernote" cols="43" rows="5" class="textarea" tabindex="5" onkeydown="limitText(this.form.short_desc,this.form.countdown1,5000);"
                          onkeyup="limitText(this.form.product_desc,this.form.countdown1,5000);">{{ $content['short_description'] }}</textarea>
                    </div>
                    <div class="mb-4">
                          ...
                          <textarea class="form-control" name="long_description" id="editor2" cols="43" rows="5" class="textarea" tabindex="5" onkeydown="limitText(this.form.short_desc,this.form.countdown1,5000);"
                            onkeyup="limitText(this.form.product_desc,this.form.countdown1,5000);">{{ $content['long_description'] }}</textarea>
                    </div>   
                  </div>
                </div>
            ...
            <div class="card w-100">
                <div class="card-body p-5">
                  ...
                  <div class="mb-4">
                    ...
                    <input type="disabled" class="form-control" data-auto-close="true" data-language="en" value="{{ date('d-M-Y', strtotime($content['created_at'])) }}" />
                  </div>
                  <div class="mb-4">
                    <label for="form-product/seo-title" class="form-label" >Updated date</label > 
                    <input type="disabled" class="form-control" data-auto-close="true" data-language="en" value="{{ date('d-M-Y', strtotime($content['updated_at'])) }}" /><label class="form-check mb-0" >
                  </div>
                </div>
                @if(count($images) > 0)
                <div class="card-body p-5">
                    ...
                    <table class="sa-table" id="myTables">
                        ...
                        @foreach($images as $image)
                          <tr>
                              <td>
                                <div class="sa-symbol sa-symbol--shape--rounded sa-symbol--size--lg">
                                  <a data-bs-toggle="offcanvas" href="#fileManagerDetails{{ $image['id']}}" ><img src="{{ url('wx.images/blogs/articles/'.$image['image']) }}" width="40" height="40" alt=""></a>
                                </div>
                              </td>
                              <td>
...
<div class="offcanvas offcanvas-end offcanvas-sa-card" tabindex="-1" id="fileManagerDetails{{ $image['id']}}" aria-labelledby="fileManagerDetailsLabel">
  ...
  <div class="offcanvas-body" data-simplebar="">
      <div class="border p-4 d-flex justify-content-center mb-4">
        <div class="max-w-20x"><img src="{{ url('wx.images/blogs/articles/'.$image['image']) }}" class="w-100 h-auto" width="320" height="320" alt=""/></div>
      </div>
      <div class="mb-4">
      ...
      <div class="hstack gap-3"><button type="submit" class="btn btn-primary flex-grow-1">Update</button><a href="{{ route('blog.imgdel', $image['id'] ) }}" class="btn btn-secondary flex-grow-1">Delete</a></div>
  </div>
</div>
</form>
@endforeach
...
function copyInputValue() {
  ...
  alert("Blog path to clipboard: " + inputField.value);
}

function copyPrefixedValue() {
  var inputField = document.getElementById("slugOutput");
  var prefixedValue = "http://www.luluconnect.com/blogs/" + inputField.value;
  ...
  alert('Blog path copied to clipboard: ' + prefixedValue);
}
</script>
<script>
